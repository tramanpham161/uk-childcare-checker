          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 gap-3">
              {(['yes', 'no', 'notSure'] as const).map(opt => (
                <button
                  key={opt}
                  onClick={() => updateData({ incomeInRange: opt })}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-12">
              ...
              <div className="bg-slate-50 rounded-2xl p-6 border border-slate-200 flex items-center justify-between">
                <div>
                  ...
                  <div className="text-xl font-bold text-slate-900">{results.some(r => r.id.includes('working')) ? 'Eligible working parent' : 'Universal entitlement'}</div>
                </div>
              </div>
            </div>
